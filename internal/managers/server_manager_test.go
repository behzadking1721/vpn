...
func TestSubscriptionCRUD(t *testing.T) {
	...
	retrievedSub, err := sm.GetSubscription(sub.ID)
	if err != nil {
		t.Fatalf("Failed to get subscription: %v", err)
	}
	
	if retrievedSub.ID != sub.ID {
		t.Errorf("Expected subscription ID %s, got %s", sub.ID, retrievedSub.ID)
	}
	
	if retrievedSub.Name != sub.Name {
		t.Errorf("Expected subscription name %s, got %s", sub.Name, retrievedSub.Name)
	}
	...
	sub.Name = "Updated Test Subscription"
	err = sm.UpdateSubscription(sub)
	if err != nil {
		t.Fatalf("Failed to update subscription: %v", err)
	}
	
	updatedSub, err := sm.GetSubscription(sub.ID)
	if err != nil {
		t.Fatalf("Failed to get updated subscription: %v", err)
	}
	
	if updatedSub.Name != "Updated Test Subscription" {
		t.Errorf("Expected updated subscription name 'Updated Test Subscription', got %s", updatedSub.Name)
	}
	...
	_, err = sm.GetSubscription(sub.ID)
	if err == nil {
		t.Error("Expected error when getting deleted subscription, got nil")
	}
}